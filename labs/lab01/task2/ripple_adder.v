...
module ripple_adder(
  ...
  // TODO: your four FA_Gate instances go here.
    FA_Gate fa0 (.sum(sum[0]), .cout(c1),   .a(a[0]), .b(b[0]), .cin(cin));
    FA_Gate fa1 (.sum(sum[1]), .cout(c2),   .a(a[1]), .b(b[1]), .cin(c1));
    FA_Gate fa2 (.sum(sum[2]), .cout(c3),   .a(a[2]), .b(b[2]), .cin(c2));
    FA_Gate fa3 (.sum(sum[3]), .cout(cout), .a(a[3]), .b(b[3]), .cin(c3));
